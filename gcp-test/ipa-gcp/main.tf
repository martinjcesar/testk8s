...
resource "google_compute_target_pool" "jenkins" {
    name = "jenkins-www-target-pool"
    instances = ["${google_compute_instance.nginx.*.self_link}"]
    //health_checks = ["${google_compute_http_health_check.default.name}"]
}

resource "google_compute_forwarding_rule" "jenkins" {
    name = "jenkins-www-forwarding-rule"
    target = "${google_compute_target_pool.jenkins.self_link}"
    port_range = "8181"
}
